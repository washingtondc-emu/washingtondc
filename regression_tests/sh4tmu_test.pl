...
################################################################################
#
# This script serves as a simple evaluation test for sh4tmu_test.
#
...
# sh4tmu_test which was previously taken from a real Dreamcast over dc-tool
...
# Environment variables telling where to get the sh4tmu_test.bin and dc_log.txt
# these should be a URL that can be downloaded using curl.
$dc_serial_log_url=$ENV{'SH4TMU_TEST_CAPTURE'};
$dc_sh4tmu_test_bin_url=$ENV{'SH4TMU_TEST_BIN'};
...
$WASH_LOG = "sh4tmu_test_wash_dbg_log.txt";
...
$DC_SERIAL_LOG = "sh4tmu_test_dc_log.txt";

#download the test binary and the validation capture
system("curl $dc_sh4tmu_test_bin_url > $SH4TMU_TEST_PATH") and die "could not download \"$dc_sh4tmu_test_bin_url\"";
...
sub do_ignore {
    ...
    if ($line =~ /the initial timestamp was [0-9]+ secs, [0-9]+ msecs/) {
        return 1;
    }

    if ($line =~ /the final timestamp was [0-9]+ and cur_time.msecs is [0-9]+/) {
        return 1;
    }

    if ($line =~ /[0-9]+ iterations of the loop were performed/) {
        return 1;
    }

    if ($line =~ /That comes out to [0-9]+\.[0-9]* iterations per millisecond./) {
        return 1;
    }
