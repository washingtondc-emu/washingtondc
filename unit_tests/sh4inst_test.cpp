...
public:
    ...
    // FMUL FRm, FRn
    // 1111nnnnmmmm0010
    static int do_binary_fmul_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
        ...
        ss << "FMUL FR" << src_reg_no << ", FR" << dst_reg_no << "\n";
        cmd = ss.str();
        test_prog.add_txt(cmd);
        const Sh4Prog::ByteList& inst = test_prog.get_prog();
        bios_load_binary(bios, 0, inst.begin(), inst.end());
    ...
    static int binary_fmul_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                 RandGen32 *randgen32) {
        int failure = 0;
        ...
        for (unsigned src = 0; src < 16; src++) {
            for (unsigned dst = 0; dst < 16; dst++) {
                ...
                failure = failure ||
                    do_binary_fmul_fr_fr(cpu, bios, mem, src, dst,
                                         src_val, dst_val);
            }
        }
    ...
    // FDIV FRm, FRn
    // 1111nnnnmmmm0011
    static int do_binary_fdiv_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                    unsigned src_reg_no, unsigned dst_reg_no,
                                    float src, float dst) {
        Sh4Prog test_prog;
        std::stringstream ss;
        ...
        ss << "FDIV FR" << src_reg_no << ", FR" << dst_reg_no << "\n";
        cmd = ss.str();
        test_prog.add_txt(cmd);
        const Sh4Prog::ByteList& inst = test_prog.get_prog();
        bios_load_binary(bios, 0, inst.begin(), inst.end());
    ...
    static int binary_fdiv_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                 RandGen32 *randgen32) {
        int failure = 0;
        ...
        for (unsigned src = 0; src < 16; src++) {
            for (unsigned dst = 0; dst < 16; dst++) {
                ...
                if (src == dst)
                    ...
                    dst_val = randgen32->pick_double();
                failure = failure ||
                    do_binary_fdiv_fr_fr(cpu, bios, mem, src, dst,
                                         src_val, dst_val);
            }
        }
    ...
    // FADD FRm, FRn
    // 1111nnnnmmmm0000
    static int do_binary_fadd_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                    unsigned src_reg_no, unsigned dst_reg_no,
                                    float src, float dst) {
        Sh4Prog test_prog;
        std::stringstream ss;
        ...
        ss << "FADD FR" << src_reg_no << ", FR" << dst_reg_no << "\n";
        cmd = ss.str();
        test_prog.add_txt(cmd);
        const Sh4Prog::ByteList& inst = test_prog.get_prog();
        bios_load_binary(bios, 0, inst.begin(), inst.end());
    ...
    static int binary_fadd_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                 RandGen32 *randgen32) {
        int failure = 0;
        ...
        for (unsigned src = 0; src < 16; src++) {
            for (unsigned dst = 0; dst < 16; dst++) {
                ...
                if (src == dst)
                    ...
                    dst_val = randgen32->pick_double();
                failure = failure ||
                    do_binary_fadd_fr_fr(cpu, bios, mem, src, dst,
                                         src_val, dst_val);
            }
        }
    ...
    // FCMP/GT FRm, FRn
    // 1111nnnnmmmm0101
    static int do_binary_fcmpgt_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                    unsigned src_reg_no, unsigned dst_reg_no,
                                    float src, float dst) {
        Sh4Prog test_prog;
        std::stringstream ss;
        ...
        ss << "FCMP/GT FR" << src_reg_no << ", FR" << dst_reg_no << "\n";
        cmd = ss.str();
        test_prog.add_txt(cmd);
        const Sh4Prog::ByteList& inst = test_prog.get_prog();
        bios_load_binary(bios, 0, inst.begin(), inst.end());
        ...
        *sh4_fpu_fr(cpu, dst_reg_no) = dst;
        ...
        bool t_expect = dst > src;
        bool t_actual = (bool)(cpu->reg[SH4_REG_SR] & SH4_SR_FLAG_T_MASK);
        if (t_actual != t_expect) {
            std::cout << "ERROR while running " << cmd << std::endl;
            std::cout << "expected value of T is " << t_expect << std::endl;
            std::cout << "actual value is " << t_actual << std::endl;
            std::cout << "inputs are are " << src << " and " << dst <<
                std::endl;
            return 1;
        }
    ...
    static int binary_fcmpgt_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                   RandGen32 *randgen32) {
        int failure = 0;
        ...
        for (unsigned src = 0; src < 16; src++) {
            for (unsigned dst = 0; dst < 16; dst++) {
                ...
                if (src == dst)
                    ...
                    dst_val = randgen32->pick_double();
                failure = failure ||
                    do_binary_fcmpgt_fr_fr(cpu, bios, mem, src, dst,
                                           src_val, dst_val);
            }
        }
    ...
    // FNEG FRn
    // 1111nnnn01001101
    static int do_unary_fneg_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
    ...
    static int unary_fneg_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                              RandGen32 *randgen32) {
        unsigned reg_no;
        int failure = 0;
        ...
        for (reg_no = 0; reg_no < 16; reg_no++) {
            failure = failure ||
                do_unary_fneg_fr(cpu, bios, mem, reg_no,
                                 randgen32->pick_double());
        }
    ...
    // FSQRT FRn
    // 1111nnnn01101101
    static int do_unary_fsqrt_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                unsigned reg_no, float in_val) {
        Sh4Prog test_prog;
        std::stringstream ss;
        std::string cmd;
    ...
    static int unary_fsqrt_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                              RandGen32 *randgen32) {
        unsigned reg_no;
        int failure = 0;
        ...
        for (reg_no = 0; reg_no < 16; reg_no++) {
            failure = failure ||
                do_unary_fsqrt_fr(cpu, bios, mem, reg_no,
                                  randgen32->pick_double(0.0, 1024.0 * 1024.0));
        }
    ...
    static int do_binary_fcmpeq_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                    unsigned src_reg_no, unsigned dst_reg_no,
                                    float src, float dst) {
        Sh4Prog test_prog;
        std::stringstream ss;
        ...
        ss << "FCMP/EQ FR" << src_reg_no << ", FR" << dst_reg_no << "\n";
        cmd = ss.str();
        test_prog.add_txt(cmd);
        const Sh4Prog::ByteList& inst = test_prog.get_prog();
        bios_load_binary(bios, 0, inst.begin(), inst.end());
    ...
    // FCMP/EQ FRm, FRn
    // 1111nnnnmmmm0100
    static int binary_fcmpeq_fr_fr(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                   RandGen32 *randgen32) {
        int failure = 0;
        ...
        for (unsigned src = 0; src < 16; src++) {
            for (unsigned dst = 0; dst < 16; dst++) {
                ...
                if (src == dst)
                    ...
                    dst_val = randgen32->pick_double();
                failure = failure ||
                    do_binary_fcmpeq_fr_fr(cpu, bios, mem, src, dst,
                                           src_val, dst_val);

                // make sure the case where they're equal gets exercised
                failure = failure ||
                    do_binary_fcmpeq_fr_fr(cpu, bios, mem, src, dst,
                                           src_val, src_val);
            }
    ...
    // FMOV DRm, XDn
    // 1111nnn1mmm01100
    static int do_binary_fmov_dr_xd(Sh4 *cpu, BiosFile *bios,
                                    Memory *mem, unsigned src_reg_no,
                                    unsigned dst_reg_no, double val) {
        Sh4Prog test_prog;
        std::stringstream ss;
        ...
        ss << "FSCHG\n";
        ss << "FMOV DR" << src_reg_no << ", XD" << dst_reg_no << "\n";
        cmd = ss.str();
        test_prog.add_txt(cmd);
        const Sh4Prog::ByteList& inst = test_prog.get_prog();
        bios_load_binary(bios, 0, inst.begin(), inst.end());
        ...
        *sh4_fpu_dr(cpu, src_reg_no >> 1) = val;
        ...
        double actual_val = *sh4_fpu_xd(cpu, dst_reg_no >> 1);
        if (actual_val != val) {
            std::cout << "ERROR while running " << cmd << std::endl;
            std::cout << "expected value of XD" << dst_reg_no << " is " <<
                val << std::endl;
            std::cout << "actual value is " << actual_val << std::endl;
            return 1;
        }
    ...
    static int binary_fmov_dr_xd(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                 RandGen32 *randgen32) {
        int failure = 0;
        ...
        for (unsigned src_reg = 0; src_reg < SH4_N_DOUBLE_REGS; src_reg++) {
            for (unsigned dst_reg = 0; dst_reg < SH4_N_DOUBLE_REGS; dst_reg++) {
                ...
                failure = failure ||
                    do_binary_fmov_dr_xd(cpu, bios, mem,
                                         src_reg * 2, dst_reg * 2, f_val);
            }
        }
    ...
    // FMOV XDm, @-Rn
    // 1111nnnnmmm11011
    static int do_fmov_binary_xd_inddecgen(Sh4 *cpu, BiosFile *bios,
                                           Memory *mem, unsigned reg_src,
                                           unsigned reg_dst, addr32_t addr,
                                           double val) {
        Sh4Prog test_prog;
    ...
    static int fmov_binary_xd_inddecgen(Sh4 *cpu, BiosFile *bios, Memory *mem,
                                        RandGen32 *randgen32) {
        int failed = 0;
        ...
        for (unsigned reg_src = 0; reg_src < SH4_N_DOUBLE_REGS; reg_src++) {
            for (unsigned reg_dst = 0; reg_dst < 16; reg_dst++) {
                ...
                double val = randgen32->pick_double();
                failed = failed ||
                    do_fmov_binary_xd_inddecgen(cpu, bios, mem, reg_src * 2,
                                                reg_dst, addr, val);
            }
        }
...
struct inst_test {
    ...
    { "binary_fmul_fr_fr", &Sh4InstTests::binary_fmul_fr_fr },
    { "binary_fdiv_fr_fr", &Sh4InstTests::binary_fdiv_fr_fr },
